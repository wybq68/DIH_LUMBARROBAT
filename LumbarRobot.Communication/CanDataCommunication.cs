...
{
    public class CanDataCommunication : IDataCommunication
    {
        const uint VCI_USBCAN1 = 3;
        ...
        private void Recieve()
        {
            ...
            while (isOpen)
            {
                try
                {
                    do
                    {
                        len = VCI_Receive(VCI_USBCAN1, DeviceInd, CNDInd, ref frameinfos[0], 1, 0);
                        if (len <= 0)
                        {
                            VCI_ReadErrInfo(VCI_USBCAN1, DeviceInd, CNDInd, ref errinfo);
                        }
                        else
                        {
